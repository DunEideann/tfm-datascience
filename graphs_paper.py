...
FIGS_PATH = '/lustre/gmeteo/WORK/reyess/figs/paper-february/'
...
# DATOS OBSERVACION
    ...
    for period, data_metrics in whole_obs_metrics.items():
        ...
        utils.metricsGraph(datasets_metrics=data_metrics, figs_path=FIGS_PATH, vmin=[5, 15, -8], vmax=[25, 35, 12], pred_type='observation_whole', fig_num = fig_num, period = period)#, extension='png')
        utils.metricsGraph(datasets_metrics=data_metrics, figs_path=FIGS_PATH, vmin=[5, 15, -8], vmax=[25, 35, 12], pred_type='observation_whole', fig_num = fig_num, period = period, extension='png')
        fig_num += Decimal('0.1')
    ...
    utils.stdGraphs(std_metrics=std_metrics, figs_path=FIGS_PATH, vmin=[0, 0], vmax=[1, 1], pred_type='standard_deviation', fig_num=1, period=period, extension='png')
...
### # FIG2 # ####
if '2' in FIGS:
    vminMetric = {'Mean': (0.4, 0.05, 3), '99Percentile': (0, 0.25, 1)}
    vmaxMetric = {'Mean': (2, 0.75, 11), '99Percentile': (3, 1.25, 16)}
    

    figName = f'fig2_rmse_error_{ENSEMBLE_QUANTITY}_part1'
    ...
    rmse_bias_mean = {f'{metric}': {} for metric in metrics_2}
    rmse_bias_group_mean = {f'{metric}': [] for metric in metrics_2}
    error_std = {f'{metric}': {} for metric in metrics_2}
    for i, predictand_name in enumerate(predictands):      
        ...
        for metric in metrics_2:
            ...
            for predictand_number in predictand_numbered:
                ...
                if metric == 'Mean':
                    rmse_bias = np.sqrt((((loaded_test - loaded_test_obs)**2).mean(dim=['time']))['tasmean'])
                elif metric == '99Percentile':
                    loaded_test = loaded_test.resample(time = 'YE').quantile(0.99, dim = 'time')
                    loaded_test_obs = loaded_test_obs.resample(time = 'YE').quantile(0.99, dim = 'time')
                    rmse_bias = (np.abs(loaded_test - loaded_test_obs)).mean(['time'])['tasmean']

                rmse_bias_test.append(rmse_bias)

            rmse_bias_concat = xr.concat(rmse_bias_test, dim='member')
            rmse_bias_mean[metric][predictand_name] = rmse_bias_concat.mean(dim='member')


        # INTERQUARTIL A MANO
        for j, (metric, metric_data) in enumerate(rmse_bias_mean.items()):
            if metric == 'Mean':
            ...
            dataToPlot = metric_data[predictand_name]
            ...
            if i == 0:
                cax = fig.add_axes([0.125, 0.505 - (j * 0.435), 0.776, 0.02])#DIST DESDE IZQUIERDA/DIST DESDE ABAJO/LARDO HORI/LARGO/VERT
                cbar = plt.colorbar(im, cax, pad=0.05, spacing='uniform', orientation='horizontal')#, extend='both', extendfrac='auto', )
                ticks = np.linspace(vmin, vmax, int(np.floor(vmaxMetric[metric][2]+1-vminMetric[metric][2])) if metric=='mean' else int(np.floor(vmaxMetric[metric][2]+1-vminMetric[metric][2])))
    ...
    discreteCMAP2 = ListedColormap(continuousCMAP2(np.linspace(0, 1, 10)))
    discreteCMAPnoWhite2 = ListedColormap(continuousCMAP2(np.linspace(0, 1, 11)[1:]))

    #print(rmse_bias_mean)

    for metric in metrics_2:
        figName = f'fig2_std_error_{ENSEMBLE_QUANTITY}_{metric}_part2'
        # Crear la figura y los ejes
        fig, axes = plt.subplots(2, 2, figsize=(10, 5), sharex=False, sharey=False, subplot_kw={'projection': ccrs.PlateCarree()})

        rmse_bias_mean_list = {key: list(value.values()) for key, value in rmse_bias_mean.items()}
        error_std[metric]['all'] = xr.concat(rmse_bias_mean_list[metric], dim='member').std(dim='member')   
        for predictand_name in predictands:
            if predictand_name in predictands_group_1:
                rmse_bias_group_mean[metric].append(rmse_bias_mean[metric][predictand_name])
        #rmse_bias_mean_group_list = {key: list(value.values()) for key, value in rmse_bias_group_mean.items()}
        error_std[metric]['group1'] = xr.concat(rmse_bias_group_mean[metric], dim='member').std(dim='member')
        j = 0

        data_to_plot = {'all-std': None, 'group1-std': None}

        data_to_plot['all-std'] = error_std[metric]['all']
        data_to_plot['group1-std'] = error_std[metric]['group1']
        print(data_to_plot['all-std'])
        print(data_to_plot['all-std'].coords['lon'].values)
        print(data_to_plot['all-std'].variable)
        # print(f'{metric}all std max: {data_to_plot['all-std']['tasmean'].max().item()}')
        # print(f'{metric}group1 std max: {data_to_plot['group1-std']['tasmean'].max().item()}')
        # print(f'{metric}all std min: {data_to_plot['all-std']['tasmean'].min().item()}')
        # print(f'{metric}group1 std min: {data_to_plot['group1-std']['tasmean'].min().item()}')

        ax1 = axes[j, 0]
        ax2 = axes[j, 1]


        ax1.set_title(f'All Std', fontsize=16)
        ...
        im2 = ax2.pcolormesh(data_to_plot['group1-std'].coords['lon'].values, data_to_plot['group1-std'].coords['lat'].values,
                            data_to_plot['group1-std'],
                            transform=ccrs.PlateCarree(),
                            cmap=discreteCMAPnoWhite2,
    ...
    del rmse_bias_mean, error_std, data_to_plot, predictands_total_mean, predictands_group_mean, rmse_bias_test
...
if '3' in FIGS:
    ...
    vminMetric = {'Mean': (4.6, 0.05, 4, 1), '1Percentile': (1, 0, 1, 1), '99Percentile': (5, 0.25, 2, 3)}
    vmaxMetric = {'Mean': (8.6, 0.75, 14, 8), '1Percentile': (8, 0.5, 26, 14), '99Percentile': (13, 1.25, 22, 13)}
    for metric in metrics_3:
        ...
        for i, predictand_name in enumerate(predictands):
            ...
            for j, (metric_fig, metric_data) in enumerate(predictand_data.items()):
                ...
                continuousCMAP = plt.get_cmap('hot_r') if metric_fig == 'mean' else plt.get_cmap('summer_r')
        ...
        # Crear la figura y los ejes
        fig, axes = plt.subplots(1, 1, figsize=(8, 12), sharex=False, sharey=False, subplot_kw={'projection': ccrs.PlateCarree()})
        ...
        cax = fig.add_axes([0.125, 0.510 - 0.203, 0.776, 0.02]) #DIST DESDE IZQUIERDA/DIST DESDE ABAJO/LARDO HORI/LARGO/VERT
        cbar = plt.colorbar(im1, cax, pad=0.05, spacing='uniform', orientation='horizontal')#, extend='both', extendfrac='auto', )
        cbar.set_ticks(np.linspace(vminMetric[metric][1], vmaxMetric[metric][1], 6))
        cbar.ax.tick_params(labelsize=8)
        ...
        plt.subplots_adjust(top=0.95, bottom=0.05, wspace=0.2, hspace=0.002) #left=0.05, right=0.82, 
        plt.savefig(f'{FIGS_PATH}{figName}.png', bbox_inches='tight')
        plt.savefig(f'{FIGS_PATH}{figName}.pdf', bbox_inches='tight')
        plt.close()
...
# Filtrar las coordenadas de new_dataset
    ...
    for shape in SHAPE_NAME:
        ...
        for i, period in enumerate(periods):
            ...
            ax_99.tick_params(axis='x', labelsize=12)
        ...
        ax1.set_yticklabels(predictands*2, fontsize=14)
        ...
        ax1.text(xmax[1] - 1, y_max - 0.5, 'Mean', fontsize=14, fontweight='bold', ha='right', va='top', color='black')
        ax_99.text(xmax[1] - 0.5, y_max/2 - 0.5, 'Percentile 99', fontsize=14, fontweight='bold', ha='right', va='top', color='black')
...
if '5' in FIGS:
    ...
    for i, predictand_name in enumerate(predictands):
        ...
        predictand_data2['Mean difference'] = predictand_data['max_mean'] - predictand_data['min_mean']
        predictand_data2['99 Percentile difference'] = predictand_data['max_99'] - predictand_data['min_99']

        vmax = {'Mean difference': 1.5, '99 Percentile difference': 3}
        vmin = {'Mean difference': 0, '99 Percentile difference': 0}
...
if '6' in FIGS:
    for metric in ['Mean', '99Percentile', '1Percentile']:
        test_pred_total = {}
        gcm_pred_total = {}
        color_list = ['lightcoral', 'lightgreen', 'lightblue', 'orchid', 'bisque']
        color_num = 0
        for predictand_name in predictands:
            ...
            figName = f'fig6_temps_Mean_Ensemble{ENSEMBLE_QUANTITY}_{predictand_name}_{metric}'
            # Plotting
            fig, ax = plt.subplots(figsize=(8, 6))

            # Scatter for dataset 1
            ax.scatter(test_pred, gcm_pred, color=f'{color_list[color_num]}', label='Long (Temperature)', alpha=0.7)
            ax.plot([13, 21], [13, 21], color='black', linestyle='-', linewidth=1.5)

            # Labels and legend
            ax.set_xlim(13, 21)#ax.set_xlim(np.floor(min([da.values.item() for da in gcm_pred])), np.ceil(max([da.values.item() for da in gcm_pred])))
            ax.set_ylim(13,21)#ax.set_ylim(np.floor(min([da.values.item() for da in test_pred])), np.ceil(max([da.values.item() for da in test_pred])))
            ...
            color_num += 1
            test_pred_total[predictand_name] = test_pred
            gcm_pred_total[predictand_name] = gcm_pred
        ...
        for predictand_name in predictands:
            ax.scatter(test_pred_total[predictand_name], gcm_pred_total[predictand_name], color=f'{color_list[color_num]}', label=f'{predictand_name}', alpha=0.4)
            ax.plot([13, 21], [13, 21], color='black', linestyle='-', linewidth=1.5)
            color_num += 1
