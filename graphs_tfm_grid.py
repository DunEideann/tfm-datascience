...
#***********************************************
#********APLICACION DE SHAPE FILE *******************
if SHAPE_NAME != 'Iberia':
...
metrics = ['mean', '99quantile', '1quantile', 'std', 'over30', 'crps']
...
for predictand_name in predictands:
    ...
    if SHAPE_NAME != 'Iberia':
        obs[predictand_name] = obs[predictand_name].sel(
...
#     utils.metricsGraph(data_metrics, figs_path=FIGS_PATH, vmin=[0, 0, -5, 0, 1], vmax=[35, 40, 15, 15, 31], pred_type='observation_whole', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     utils.metricsGraph(test_obs_metrics[period], figs_path=FIGS_PATH, vmin=[0, 0, -5, 0, 1], vmax=[35, 40, 15, 15, 31], pred_type='observation_test', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     utils.metricsGraph(train_obs_metrics[period], figs_path=FIGS_PATH, vmin=[0, 0, -5, 0, 1], vmax=[35, 40, 15, 15, 31], pred_type='observation_train', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     fig_num += Decimal('0.1')
...
for predictand_name in predictands:
    ...
    if SHAPE_NAME != 'Iberia':
        # train_preds['annual'][predictand_name] = train_preds['annual'][predictand_name].sel(
        #     lat=new_dataset_filtered.lat,
        #     lon=new_dataset_filtered.lon,
        # )
        test_preds['annual'][predictand_name] = test_preds['annual'][predictand_name].sel(
            lat=new_dataset_filtered.lat,
            lon=new_dataset_filtered.lon,
        )
...
#     utils.metricsGraph(data_metrics, figs_path=FIGS_PATH, vmin=[0, 0, -5, 0, 1], vmax=[35, 40, 15, 15, 21], pred_type='prediction_whole', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     utils.metricsGraph(test_metrics[period], figs_path=FIGS_PATH, vmin=[0, 0, -5, 0, 1], vmax=[35, 40, 15, 15, 21], pred_type='prediction_test', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     utils.metricsGraph(train_metrics[period], figs_path=FIGS_PATH, vmin=[0, 0, -5, 0, 1], vmax=[35, 40, 15, 15, 21], pred_type='prediction_train', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     fig_num += Decimal('0.1')
...
for predictand_name in predictands:
    diff_test_metrics['annual'][predictand_name] = {
            key: test_metrics['annual'][predictand_name][key]-test_obs_metrics['annual'][predictand_name][key] 
                ...
                for key in metrics}
    diff_test_metrics['annual'][predictand_name]['crps'] = crps_ensemble(test_obs['annual'][predictand_name], test_preds['annual'][predictand_name].expand_dims("member"), dim='time')
    for season_name, months in seasons.items():
        diff_test_metrics[season_name][predictand_name] = {
            key: test_metrics[season_name][predictand_name][key]-test_obs_metrics[season_name][predictand_name][key] 
                ...
                for key in metrics}
        diff_test_metrics[season_name][predictand_name]['crps'] = crps_ensemble(test_obs[season_name][predictand_name], test_preds[season_name][predictand_name].expand_dims("member"), dim='time')
...
for period, data_metrics in diff_test_metrics.items():
    utils.metricsGraph(data_metrics, figs_path=FIGS_PATH, vmin=[-1.5, -1.5, -1.5, 0.5, -35, 0], vmax=[1.5, 1.5, 1.5, 1.5, 35, 1], pred_type='diff_test', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}', extension='pdf', noWhite=True, colorModifier=colorModifier, ticksX=11, numberStatistics=6)
    fig_num += Decimal('0.1')
...
#     if SHAPE_NAME != 'Iberia':
#         long_preds['annual'][predictand_name] = long_preds['annual'][predictand_name].sel(
#             lat=new_dataset_filtered.lat,
#             lon=new_dataset_filtered.lon,
#         )
...
#     utils.metricsGraph(data_metrics, figs_path=FIGS_PATH, vmin=[1, 2, 0, 0.6, 1], vmax=[11, 17, 10, 1.6, 201], pred_type='climate_signal', fig_num = fig_num, period = period, extra=f'_{SHAPE_NAME}')#, extension='png')
#     fig_num += Decimal('0.1')
